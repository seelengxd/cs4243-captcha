...
def train_model(resume_checkpoint=None, extra_epochs=30, start_epoch=0):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Training on device:", device)

    # Hyperparameters
    model_name = "BaseGateCNN"
    # num_gateblocks = 8
    # mid1_channels = 64
    # mid2_channels = 512
    input_channels = 3
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for images, targets_concat, target_lengths in dataloader:
            ...
            input_lengths = torch.full(size=(batch_size_actual,), fill_value=logits_ctc.size(0), dtype=torch.long).to(
                device)
        ...
        name = f"{model_name}_epoch{epoch + 1}_hidden{hidden_channels}_channel{input_channels}_lr{learning_rate}_batchsize{batch_size}"
...
if __name__ == "__main__":
    # resume_checkpoint = "checkpoints/CRNN_epoch30_resnet18_True_lstmhidden128_lstmlayer2_channel3_lr0.001_batchsize64.pth"
    # extra_epochs = 15
    # start_epoch = 30
    train_model()
